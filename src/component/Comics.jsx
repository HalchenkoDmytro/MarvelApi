...
const CHARACTERAPI = 'http://gateway.marvel.com/v1/public/comics';
...
function Comics() {
  ...
  React.useEffect(() => {
    const fetch = async () => {
      if (query === '') {
        ...
        const result = await axios(`${CHARACTERAPI}?titleStartsWith=${query}&ts=1&apikey=${APIKEY}&hash=${HASH}`)
        console.log(result.data.data.results)
        setItems(result.data.data.results)
        setLoading(false)
      }
  ...
  return (
    <div className="container">
      ...
      <ComicsList items={items} loading={loading} />
    </div>
  );
}
